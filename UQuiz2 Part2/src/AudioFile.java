...
/**
 * This class models the AudioFile data type. It contains information about the title and artist
 * of an audio file, as well as the duration of the audio in number of seconds.
 * 
 * NOTES:
...
public class AudioFile {
  ...
  private static ArrayList<String> registeredArtists = new ArrayList<String>(Arrays.asList("Beyonce", 
      "Hozier", "Parliament-Funkadelic", "Cecil Baldwin", "Jonathan Sims", "Steve Shell", "Lil Nas X",
      "Nicki Minaj", "Megan Thee Stallion", "SZA", "The HU", "Drake", "Bastille", "DISPATCH"));
  ...
  /**
   * Returns a string representation of this AudioFile. The returned string has the following format:
   * "<[title]> by [artist] ([duration])"
   * For example:
   *   <ALIEN SUPERSTAR> by Beyonce (3:40)
   *   <Nina Cried Power> by Hozier (3:45)
   *   <One Nation Under A Groove> by Parliament-Funkadelic (11:26)
   *   
  ...
  public String toString() {
    // 10. Complete the implementation of this method
	  return "<" + title + "> by " + artist + " (" + getDuration() + ")";
  }
}
 ...
 * There is no tester or main method for this class.
 */
class PodcastEpisode extends AudioFile { // TODO: 11. Set this class to be a subclass of the AudioFile class
  ...
  public PodcastEpisode(String seriesName, int episodeNumber, String title, String artist, 
    ...
    // 13. Check the validity of the episode number parameter
	  if (episodeNumber <= 0) {
		  throw new IllegalStateException("episodeNumber cannot be less than 0");
	  }
   ...
   * ArrayList:
   * [seriesName] episode [episodeNumber]: <[title]> by [artist] ([duration]) - listened
   * 
   * Note that the <[title]> by [artist] ([duration]) is the result of calling the super class'
   * toString() method.
   * 
   * Examples:
   *   WTNV episode 19: <Sandstorm> by Cecil Baldwin (26:39) - listened
   *   TMA episode 160: <The Eye Opens> by Jonathan Sims (32:33)
   *   Old Gods episode 12: <The Other Queen> by Steve Shell (27:29) - listened
   *   
  ...
  public String toString() {
    ...
    // 16. Complete the implementation of this method
	  String episodeString = String.format("%s episode %d: %s", seriesName, episodeNumber, super.toString());
	    if (playedAudioFiles.contains(this)) {
	        episodeString += " - listened";
	    }
	    return episodeString;
	}
    // Note: call the super class implementation of toString() to get the title, artist, and duration
