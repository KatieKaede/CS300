...
public class Bookshelf {
  ...
  public static void add(String toAdd) {
    ...
    // 3. Throw a new IllegalStateException if the oversize array books is full
	  ...
	  if (size == books.length) {
		  throw new IllegalStateException("Can't add book, the shelf is full");
	  }
  ...
  public static boolean testAddNonFullArray() {
    ...
    books = new String[] {"sample", "firstBook", "secondBook", null};
    size = 3;
    ...
    String[] expectedBooks = {"sample", "firstBook", "secondBook", newTitle}; 

    // 4. Check for bugs.
    // Expected behavior: The new book should be added at the end of the array books and size
    // should be 4.
    
    if ((Arrays.deepEquals(books, expectedBooks) && size == 4) == true);
    ...
    return true; // default return statement added to resolve compiler errors. You can change it.
  }
