...
public class ExceptionalCareTester {
  ...
  /**
   * This method tests the ExceptionalCareAdmissions constructor by verifying that it throws an
   * IllegalArgumentException when called with a capacity less than or equal to 0.
   * First, it resets the patient counter to 1. Then, it attempts to create an instance of
   * ExceptionalCareAdmissions with a negative capacity value. If the constructor throws an
   * IllegalArgumentException, the test method returns true, indicating that the test passed. 
   ...
   * Then it tests three different cases where it expects to get the correct index of a new PatientRecord
   * to be added to the Admissions list.
  ...
  /**
   * A test case that tests the basic accessors (isFull(), size(), and getNumberofSeenPatients)
   ...
   * checks if the method properly marks a patient as seen and updates the number of seen patients.
   * It creates an ExceptionalCareAdmissions object and saves a shallow copy of at least one of 
   * the PatientRecord references. It then calls seePatient() on the caseID of the saved reference
   * and verifies that its hasBeenSeen() accessor return the correct value changes.
   * Then tests the getNumberSeenPatients() method before and after seeing a different patient.
   * @return  true if the method passes the test, otherwise it returns false.
   */
  ...
  /**
   * Tests the ExceptionalCareAdmissions class's seePatient() method when it is called with an invalid caseID.
   * The test verifies that when an invalid caseID is passed to the seePatient() method,
   * an IllegalArgumentException is thrown. This method creates an ExceptionalCareAdmissions 
   * object and adds two PatientRecords to it. It then tests to see a patient
   * with a caseID that is not present. 
  ...
  public static boolean testGetSummary() {
	  ...
	  // used the "test getSummary using a patientsList with multiple patients at a single triage level"
	  ExceptionalCareAdmissions admissions = new ExceptionalCareAdmissions(5);
	  PatientRecord p1 = new PatientRecord('M', 22, PatientRecord.RED);
  ...
  public static boolean testCleanList() {
	    ...
	    // test case 1: verify that a patientsList with no seen patients does not change after calling cleanPatientsList()
	    ...
	    // test case 2: verify that patients who have been seen are removed from the list after calling cleanPatientsList()
	    //UGH.cleanPatientsList(new File("fileclean.txt"));
